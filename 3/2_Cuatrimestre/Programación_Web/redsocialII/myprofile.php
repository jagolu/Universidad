<!doctype html>
	...
	<body>
		...
		<section id="subirHistoria">
			<div>
				<?php
					$conexion = mysqli_connect("localhost", "pw_77148079", "Xpw_77148079");
						...
						$tildes=$conexion->query("SET NAMES 'utf8'");
						$usuario=$_SESSION['usuario'];
						$result=mysqli_query($conexion, "select nombreUsuario, imagen, hayImagen from usuarios where nombreUsuario='".$usuario."'");
						$extraido=mysqli_fetch_array($result);
						echo '<h2 id="nombre">'.$usuario.'</h2>';
						if($extraido['hayImagen']){
							echo '<img src="data:image/jpg;base64,';
							echo base64_encode($extraido['imagen']);
							echo '">';
			...
			<form action="form.php" method="post" id="formSubirHistoria" enctype="multipart/form-data" onsubmit="return comprobarSubirHistoria()">
				<input type="text" id="inputTitulo" name="tituloSubirHistoria" placeholder="Introduce aquí el título de la entrada">
				<input type="text" id="inputEntrada" name="entradaSubirHistoria" placeholder="Introduce aquí tu nueva entrada">
				<input type="file" id="subirImagenEntrada" name="imagenSubirHistoria" value="Subir Imagen">
				<input type="submit" id="enviarEntrada" value="Enviar">
			</form>
		</section>
		<section id="historias">
			<?php 
				...
				$tildes=$conexion->query("SET NAMES 'utf8'");
				$usuario=$_SESSION['usuario'];
				$result=mysqli_query($conexion, "select entradas.*, usuarios.imagen, usuarios.hayImagen from entradas, usuarios where usuario='".$usuario."' and usuarios.nombreUsuario=entradas.usuario order by entradas.id desc;");
				$n=$_GET["n"];
				$hasta=($n+1)*8;
				$historiasHechas=0;
				for($i=0;$i<($n*8);$i++){
